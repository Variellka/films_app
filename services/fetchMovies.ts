...
import { IGenre, IMoviesResponse } from "../utils/types";
...
<IMoviesResponse, void, ThunkConfig>('movies/fetchMovies', async (_, thunkApi) => {
    ...
    try {
        const response = await axios.get(url, {...options, params: {
            language: 'en-US',
            with_genres: genres?.map((item: IGenre) => item.id).join(),
